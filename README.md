# 🌌 Personal-WebPage
A space-themed personal webpage built with HTML and CSS, showcasing my interests, ideas, skills, and creative side.

---

## 🌐 Visit the Website

The website is available through two hosting platforms:

- [✨ Visit using GitHub Pages](https://rz9876.github.io/Personal-WebPage/)
- [🚀 Visit using InfinityFree](https://razan.free.je/?i=1)
  
[🟥 I added a file in Arabic called "Notes to Myself" in case you would like to read it.](ملاحظات%20تفصيلية%20لنفسي.pdf)

---

## 📸 Website Preview

### Main Banner Section
<img width="1242" height="731" alt="Main banner section of My Little Universe website" src="https://github.com/user-attachments/assets/afadf2d4-b099-4b55-bb2d-4d87a053fd69" />

### About Me Section
<img width="1240" height="722" alt="About Me section of My Little Universe website" src="https://github.com/user-attachments/assets/191292d1-d78a-47fb-aeb1-0a2e2efbf081" />

### My Worlds Section
<img width="1244" height="724" alt="My Worlds section of My Little Universe website" src="https://github.com/user-attachments/assets/ac09dbb0-21e8-40dd-900e-2bd01f73ae6e" />

### Ideas Section
<img width="1248" height="720" alt="Ideas section of My Little Universe website" src="https://github.com/user-attachments/assets/76eedc69-4844-4bbb-ad00-83a52214be7d" />

### Skills Section
<img width="1239" height="723" alt="Skills section of My Little Universe website" src="https://github.com/user-attachments/assets/6eb0ddb1-62c8-402f-a124-1311a1c62d14" />

### Final Section
<img width="1238" height="589" alt="Final section of My Little Universe website" src="https://github.com/user-attachments/assets/768c9288-999c-4b02-8bcd-51bf4beda121" />

---

## 💫 About the Project

**My Little Universe** is a personal website created to introduce who I am, what I am interested in, and the ideas I hope to bring to life.

The website uses a space-inspired theme with stars, glowing objects, moving planets, glass-style cards, animations, and creative visual elements.

---

## 🪐 Website Sections

- **Main Banner** — The main introduction to the website
- **A Little About Me** — Information about my field of study and interests
- **The Worlds Inside Me** — The Builder, The Explorer, and The Dreamer
- **Ideas I Want to Bring to Life** — Ideas related to technology, health, and creative digital experiences
- **Currently Loading** — A visual representation of the skills I am developing
- **Final Message** — A closing message about continuing to learn and grow

---

## ✨ Features

- Smooth navigation between sections
- Space-themed background
- Decorative stars and animated planets
- Floating animated words
- Glass-style cards
- Hover effects
- Animated progress bars
- Google Fonts
- Semantic HTML structure
- Organized external CSS file

---

## 🛠️ Technologies Used

- HTML
- CSS
- Google Fonts

No JavaScript or external framework was used.

